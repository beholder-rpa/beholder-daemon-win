namespace beholder_occipital.ObjectDetection
{
  ...
  {
    private readonly ILogger<SiftFlannMatchProcessor> _logger;

    public SiftFlannMatchProcessor(ILogger<SiftFlannMatchProcessor> logger)
    {
    ...
    public DMatch[][] ProcessAndObtainMatches(Mat queryImage, Mat trainImage, out KeyPoint[] queryKeyPoints, out KeyPoint[] trainKeyPoints)
    {
      ...
      {
        _logger.LogError($"Error obtaining matches: {ex.Message}", ex);
