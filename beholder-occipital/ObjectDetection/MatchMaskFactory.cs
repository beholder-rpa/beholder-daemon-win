namespace beholder_occipital.ObjectDetection
{
  ...
  public class MatchMaskFactory : IMatchMaskFactory
  {
    ...
    private int ApplyScaleRotationMatchCulling(DMatch[][] matches, KeyPoint[] queryKeyPoints, KeyPoint[] trainKeyPoints, Mat mask, float scaleIncrement = 1.5f, int rotationBins = 20)
    {
      int idx = 0;
      int nonZeroCount = 0;
      ...
      int[] channels = { 0, 1 };
      if (scaleRanges[0] < scaleRanges[1])
      {
        return nonZeroCount;
      }
      ...
      {
        _logger.LogError($"An error occurred while attemping to perform scale rotation with culling: {ex.Message}", ex);
      }
